...
def ldm_date(env, day):
    """A date as people write it, in the reader's language: 16 October 2026."""
    return format_date(env, day, date_format="d MMMM y") if day else ""
...
class LegalAppealRule(models.Model):
    ...
    start_event = fields.Selection(
        [
            ...
            ("discovery", "Discovery"),
            ("event", "The event"),
            ("hearing_outcome", "Session outcome"),
            ("previous_deadline_end", "End of the previous period"),
        ],
        help="A period counts from the day after this event (Civil Procedure Law Art. 25(1)). End of the "
        "previous period: it starts when the period before it ends without an answer, since silence counts "
        "as a rejection.")
