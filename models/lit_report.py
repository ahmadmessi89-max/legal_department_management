...
class ReportLdmHearingRoll(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        hearings = hearings.sorted(lambda h: (h.department_id.name or "", self._ldm_attendee(h), h.date, h.time,
                                              h.id))
        groups = []
        for hearing in hearings:
            ...
            person = self._ldm_attendee(hearing)
            if not lawyers or lawyers[-1]["name"] != person:
                lawyers.append({"name": person, "rows": []})
        ...
        return {
            ...
            "printed_on": ldm_date(self.env, fields.Date.context_today(self)),
        }
    ...
    @api.model
    def _ldm_attendee(self, hearing):
        """Who stands before the court: the lawyer attending, else the outside
        lawyer attending by substitution."""
        if hearing.attending_user_id:
            if hearing.substitute_partner_id:
                return _("%(lawyer)s, by substitution %(substitute)s", lawyer=hearing.attending_user_id.name,
                         substitute=hearing.substitute_partner_id.name)
            return hearing.attending_user_id.name
        if hearing.substitute_partner_id:
            return _("%s (by substitution)", hearing.substitute_partner_id.name)
        return _("Nobody assigned")
    ...
    def _ldm_row(self, hearing):
        ...
        return {
            ...
            "against": _("against %s", task.opponent_name) if task.opponent_name else "",
            "purpose": hearing.purpose or "",
            "room": _("Room %s", hearing.court_room) if hearing.court_room else "",
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for hearing in hearings:
            ...
            letters.append({
                ...
                "today": ldm_date(self.env, fields.Date.context_today(self)),
    ...
    def _ldm_body(self, hearing, principal, substitute):
        ...
        if poa:
            values.update(number=poa.number or "", notary=poa.notary_office or "",
                          issued=ldm_date(self.env, poa.date_issued) if poa.date_issued else "")
