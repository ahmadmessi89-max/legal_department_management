...
class LegalTask(models.Model):
    ...
    def get_my_day(self, scope="me"):
        ...
        return {
            ...
            "approvals": approvals,
            "manager": manager,
            "counts": self._ldm_md_counts(ctx),
            "all_clear": {
    ...
    def _ldm_md_row(self, task, kind, day, reason, action=False, who="", key=None, extra=None):
        row = {
            ...
            "action": action,
            # The row's date pill already says overdue or due within 48 hours;
            # the flags say what else is wrong with the matter.
            "flags": task._ldm_flags(),
        }
        if extra:
            row.update(extra)
        return row
        ...
        for deadline in self.env["legal.deadline"].search(domain, order="date_safe", limit=100):
            ...
            if task:
                ...
                rows.append({
                    ...
                    "line": "", "reason": deadline.name, "urgent": False, "confidential": False, "flags": [],
        ...
        for activity in Activity.search(domain, order="date_deadline", limit=100):
            ...
            if activity.res_model == "legal.task":
                ...
                row = {
                    ...
                    "urgent": False, "confidential": False, "who": "", "task_id": False, "flags": [],
            ...
            for check in checks:
                if check.task_id:
                    ...
                    row = {"key": f"conflict-{check.id}", "kind": "conflict", "date": False,
                           "subject": check.query, "number": "", "line": "", "urgent": False, "flags": [],
    ...
    def _ldm_md_counts(self, ctx):
        """The stat chips: a handful of counts, each opening the records it
        counted. Mine for lawyers and clerks, the scope chosen for managers,
        ...
        counts = [
            {"key": "open", "label": _("Open matters"), "count": total, "tone": "brand", "icon": "folder-open",
        ...
        if (ctx["is_manager"] or ctx["read_only"]) and self._ldm_feature("group_ldm_approvals"):
            ...
            counts.append({"key": "approval", "label": _("Awaiting approval"), "count": waiting, "tone": "violet",
