...
class LegalTask(models.Model):
    ...
    @api.depends("state", "approval_state", "next_date", "court_stage", "date_state_changed", "expenses_amount",
                 ...
                 "due_date", "date_opened", "date_submitted", "waiting_on", "is_urgent", "template_id",
                 "department_id", "missing_document_count")
    def _compute_ldm_cockpit(self):
        ...
        for task in self:
            ...
            task.ldm_cockpit = {
                ...
                "flags": task._ldm_flags(task.next_date, today) if task.state not in CLOSED_STATES else [],
                "suggestion": task._ldm_suggested_target(today) if can_write else False,
    ...
    # ------------------------------------------------------------------
    # Flags (design direction: the mockup's row flags, from real data only)
    # ------------------------------------------------------------------
    def _ldm_flags(self, day=None, today=None):
        """Pills that say what is wrong with this matter now: overdue or due
        within 48 hours (for ``day``), urgent, at the body past its usual
        answer time, documents still missing."""
        self.ensure_one()
        ...
        if day:
            ...
            if delta < 0:
                flags.append({"key": "overdue", "label": _("Overdue"), "tone": "danger", "icon": "triangle-alert"})
            elif delta <= 2:
                flags.append({"key": "due_48h", "label": _("Due within 48 h"), "tone": "warning", "icon": "clock"})
        if self.is_urgent:
            flags.append({"key": "urgent", "label": _("Urgent"), "tone": "danger", "icon": "flag"})
        target = self.template_id.target_days or self.department_id.target_days
        if (target and self.date_submitted and self.waiting_on in ("body", "verification")
                and self.days_at_body > target):
            flags.append({"key": "past_target", "tone": "warning", "icon": "hourglass",
                          "label": _("At the body %(days)s working days (usual %(target)s)",
                                     days=self.days_at_body, target=target)})
        if self.missing_document_count:
            flags.append({"key": "missing_documents", "tone": "warning", "icon": "file-x",
                          "label": _("Documents missing: %s", self.missing_document_count)})
        return flags

    # ------------------------------------------------------------------
    # Suggested target date (design direction: the mockup's "AI deadline"
    # made real: arithmetic on the matter type and the body, never a guess)
    # ------------------------------------------------------------------
    def _ldm_suggested_target(self, today=None):
        """The target date the matter type and the body imply, with the
        reasons, when it differs from the target date set on the matter.
        Lawsuits follow the court's dates and get no suggestion."""
        self.ensure_one()
        if self.state in CLOSED_STATES or self.kind in ("litigation", "execution"):
            return False
        today = today or fields.Date.context_today(self)
        company = self.company_id
        body = self.department_id
        calendar = body._ldm_calendar() if body else company._ldm_calendar()
        candidates, reasons = [], []
        duration = self.template_id.duration_days
        if duration:
            start = self.date_opened or today
            candidates.append(company.ldm_add_working_days(start, duration, calendar))
            reasons.append(_("%(type)s: %(days)s working days from opening on %(date)s",
                             type=self.template_id.name, days=duration, date=format_date(self.env, start)))
        answer = self.template_id.target_days or body.target_days
        if answer:
            planned = self.step_ids.filtered(lambda s: s.is_visit and s.date_due).sorted("date_due")[:1]
            start = self.date_submitted or planned.date_due or today
            candidates.append(company.ldm_add_working_days(start, answer, calendar))
            where = body.name or _("The body")
            if self.date_submitted:
                reasons.append(_("%(body)s usually answers in %(days)s working days; submitted on %(date)s",
                                 body=where, days=answer, date=format_date(self.env, start)))
            else:
                reasons.append(_("%(body)s usually answers in %(days)s working days; submission planned for %(date)s",
                                 body=where, days=answer, date=format_date(self.env, start)))
        if not candidates:
            return False
        suggested = max(candidates)
        if suggested == self.due_date:
            return False
        return {
            "date": iso(suggested),
            "current": iso(self.due_date),
            "days": (suggested - today).days,
            "reasons": reasons,
        }

    def action_ldm_use_suggested_target(self):
        """Set the target date to the suggestion shown on the cockpit."""
        self.ensure_one()
        self.check_access("write")
        suggestion = self._ldm_suggested_target()
        if not suggestion:
            raise UserError(_("There is no other target date to suggest for %s.", self.display_name))
        self.due_date = fields.Date.to_date(suggestion["date"])
        return True
    ...
    def ldm_approval_preview(self):
        ...
        return {
            ...
            "title": task.name,
            "client": task.legal_company_id.sudo().name or "",
            "type": task.template_id.name or kinds.get(task.kind, ""),
            "body": task.department_id.sudo().name or "",
            "responsible": task.lawyer_id.sudo().name or "",
            "requested_by": task.approval_requested_by_id.sudo().name or "",
    ...
    def _ldm_agenda_items(self, start, end, scope, limit=300):
        ...
        for hearing in Hearing.search(domain, order="date, time", limit=limit):
            ...
            items.append({
                ...
                "matter_kind": task.kind or "other",
        ...
        domain = [("state", "=", "todo"), ("is_visit", "=", True), ("date_due", ">=", start), ("date_due", "<=", end),
                  ("task_id.state", "in", OPEN_STATES), ("task_id.approval_state", "!=", "to_approve")]
        if mine:
        ...
        for deadline in Deadline.search(domain, order="date_safe", limit=limit):
            ...
            items.append({
                ...
                "matter_kind": task.kind if task else "other",
