...
def matter_line(task):
    ...
    Read with sudo on the two related records only: whoever may see the matter
    sees these two names on it in every native view already (an approver, for
    one, sees a matter awaiting approval without being on its client's team)."""
    parts = [task.legal_company_id.sudo().name or "", task.department_id.sudo().name or ""]
