...
class LegalHearing(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for hearing in self:
            if calendar:
                ...
                parts = [format_date(self.env, hearing.date) if hearing.date else "", _time_label(hearing.time),
    ...
    def _ldm_date_warning(self, day):
        ...
        if company.ldm_is_working_day(day, self._ldm_calendar()):
            return False
        return _("%s is not a working day at this court.", format_date(self.env, day))
