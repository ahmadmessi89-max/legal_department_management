...
class LegalTask(models.Model):
    ...
    @api.depends("engagement_id.signed", "engagement_id.state", "state")
    def _compute_ldm_agreement(self):
        for task in self:
            engagement = task.engagement_id
            task.ldm_no_signed_agreement = task.state not in ("draft", "cancelled") and not (
                engagement and engagement.signed and engagement.state != "draft")

    def _search_ldm_no_signed_agreement(self, operator, value):
        if operator not in ("=", "!=") or not isinstance(value, bool):
            return NotImplemented
        signed = Domain("engagement_id.signed", "=", True) & Domain("engagement_id.state", "!=", "draft")
        missing = Domain("state", "not in", ("draft", "cancelled")) & ~signed
    ...
    def action_ldm_message(self):
        self.ensure_one()
        return self.env["legal.client.message.wizard"].ldm_open(False, task=self)
