...
from .lit_rules import LAW_OF_BRANCH, ldm_date, ldm_user_is
...
class LegalJudgment(models.Model):
    ...
    def _compute_deadline_info(self):
        for judgment in self:
            ...
            if not first:
                judgment.next_deadline_label = False
            elif first.date_safe:
                judgment.next_deadline_label = _("Act by %s", ldm_date(self.env, first.date_safe))
            else:
                judgment.next_deadline_label = _("Waiting for the notification date")

    @api.depends("date", "result", "task_id.task_number")
    def _compute_display_name(self):
        results = dict(self._fields["result"]._description_selection(self.env))
        for judgment in self:
            parts = [_("Judgment"), ldm_date(self.env, judgment.date) if judgment.date else "",
    ...
    def _ldm_post_changes(self, created, moves, stale):
        ...
        for deadline, old_safe, new_safe, _old_legal, new_legal in moves:
            ...
            lines.append(_("%(now)s (was %(old)s)", now=now, old=ldm_date(self.env, old_safe)) if old_safe else now)
        ...
        body = Markup("<p>%s</p><ul>%s</ul>") % (
            _("Deadlines of the judgment of %s recounted:", ldm_date(self.env, self.date)),
        ...
        for judgment in self:
            ...
            judgment.task_id.message_post(
                body=_("The judgment of %(date)s became final on %(final)s: no challenge was lodged in time.",
                       date=ldm_date(self.env, judgment.date), final=ldm_date(self.env, judgment.final_date)),
