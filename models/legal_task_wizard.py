...
class LegalTaskCreateWizard(models.TransientModel):
    ...
    suggested_date = fields.Date(string="Suggested target date", compute="_compute_suggested_date")
    suggested_note = fields.Char(compute="_compute_suggested_date")
    ...
    @api.depends("template_id", "department_id", "company_id")
    def _compute_suggested_date(self):
        """A real calculation, never a guess: the type's expected duration plus
        the body's usual answer time, counted in working days on the body's
        calendar (Friday, Saturday and public holidays skipped)."""
        today = fields.Date.context_today(self)
        for wizard in self:
            wizard.suggested_date = False
            wizard.suggested_note = False
            template = wizard.template_id
            if not template or template.kind in KEY_DATE_KINDS:
                continue
            body = wizard.department_id or template.department_id
            duration = template.duration_days or 0
            at_body = (body.target_days if body else 0) or template.target_days or 0
            if not duration + at_body:
                continue
            company = wizard.company_id or self.env.company
            calendar = body._ldm_calendar() if body else company._ldm_calendar()
            wizard.suggested_date = company.ldm_add_working_days(today, duration + at_body, calendar)
            parts = []
            if duration:
                parts.append(_("%s working days for this type", duration))
            if at_body:
                parts.append(_("%s working days at the body", at_body))
            wizard.suggested_note = " + ".join(parts)
