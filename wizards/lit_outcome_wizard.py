...
from ..models.lit_rules import LAW_OF_BRANCH, ldm_date, ldm_user_is
...
class LegalHearingOutcomeWizard(models.TransientModel):
    ...
    next_time = fields.Float(string="At", compute="_compute_next_time", store=True, readonly=False, precompute=True,
                             help="Courts usually keep the session's time; change it if the court set another.")
    ...
    def _compute_next_time(self):
        for wizard in self:
            wizard.next_time = wizard.hearing_id.time
    ...
    def _compute_period_preview(self):
        for wizard in self:
            ...
            wizard.period_preview = wizard._ldm_dates_label(safe, legal, prefix=False)
    ...
    def _ldm_dates_label(self, safe, legal, prefix=True):
        if not safe:
            return ""
        if legal and legal != safe:
            if prefix:
                return _("act by %(safe)s · legal last day %(legal)s", safe=ldm_date(self.env, safe),
                         legal=ldm_date(self.env, legal))
            return _("%(safe)s · legal last day %(legal)s", safe=ldm_date(self.env, safe),
                     legal=ldm_date(self.env, legal))
        return _("act by %s", ldm_date(self.env, safe)) if prefix else ldm_date(self.env, safe)
    ...
    def _onchange_main_outcome(self):
        ...
        # A case is left for review when both sides stay away (CCP Art. 54).
        if self.main_outcome == "left_for_review":
            self.attendance = "both_absent"
    ...
    def action_confirm(self):
        ...
        task = hearing.task_id
        lines = [_("Court session of %(date)s: %(outcome)s.", date=ldm_date(self.env, hearing.date),
        ...
        if self.next_date and outcome not in NO_NEXT:
            ...
            lines.append(_("Next session: %s.", ldm_date(self.env, self.next_date)))
        ...
        if self.needed_before and next_hearing:
            ...
            task.activity_schedule(
                ...
                note=_("Needed before the court session of %s.", ldm_date(self.env, self.next_date)))
        ...
        message = lines[0] if not next_hearing else _("Recorded. Next session on %s.",
                                                      ldm_date(self.env, self.next_date))
        return {
