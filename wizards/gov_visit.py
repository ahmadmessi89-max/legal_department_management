...
def ldm_format_amount(env, amount, currency):
    """An amount with its currency symbol; Iraqi dinars in whole dinars (SPEC 14.3)."""
    digits = 0 if currency.name == "IQD" else currency.decimal_places
    number = formatLang(env, amount, digits=digits)
    return f"{currency.symbol} {number}" if currency.position == "before" else f"{number} {currency.symbol}"
...
class LegalVisitWizard(models.TransientModel):
    ...
    def _ldm_history_line(self, step, expense, next_step):
        ...
        if expense:
            lines.append(_("Fee paid: %s", ldm_format_amount(self.env, self.fee_amount, self.currency_id)))
        if self.waiting_for:
