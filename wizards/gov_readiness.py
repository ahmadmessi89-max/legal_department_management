...
class LegalReadinessWizard(models.TransientModel):
    ...
    _description = "Documents valid on a date"
    ...
    def _ldm_open(self, context):
        return {
            ...
            "name": _("Documents valid on a date"),
    ...
    def _compute_summary(self):
        for wizard in self:
            ...
            if not wizard.legal_company_id or not wizard.date:
                ...
                wizard.summary = _("%(ready)s of %(total)s documents are valid on %(date)s.",
...
class LegalReadinessWizardLine(models.TransientModel):
    ...
    _description = "Document valid on a date"
    _order = "sequence, id"
    ...
    number = fields.Char(related="company_document_id.number", string="Number")
