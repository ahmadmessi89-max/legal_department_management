...
class TestGovReference(GovCase):
    ...
    def test_merges_into_bodies_spelled_differently(self):
        ...
        services = self.env["legal.task.template"].with_context(active_test=False).search(
            [("ldm_ref_code", "=like", "SVC-%")])
        services.write({"ldm_ref_code": False, "name": "Neutralised service"})
        for service in services:
            service.update_field_translations("name", {"ar_001": "Neutralised service"})
