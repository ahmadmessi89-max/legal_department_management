...
class TestLitCourt(LitCase):
    ...
    def test_hearing_roll_by_court_and_lawyer(self):
        self.hearing(date=self.d("2026-10-04"), time=9.5, purpose="Pleadings")
        self.hearing(date=self.d("2026-10-05"), department_id=self.appeal_court.id, attending_user_id=self.lawyer2.id)
        self.hearing(date=self.d("2026-11-20"))
        Report = self.env["report.legal_department_management.report_ldm_hearing_roll"]
        values = Report._get_report_values([], {"date_from": "2026-10-01", "date_to": "2026-10-07"})
        ...
        html = self.env["ir.actions.report"]._render_qweb_html(
            ...
            data={"date_from": "2026-10-01", "date_to": "2026-10-07"})[0].decode()
        self.assertIn(self.appeal_court.name, html)
        self.assertIn("Pleadings", html)
        # From a selection of sessions
        ...
        wizard = self.env["legal.hearing.roll.wizard"].with_user(self.lawyer).create({
            "date_from": self.d("2026-10-01"), "date_to": self.d("2026-10-07")})
        action = wizard.action_print()
        self.assertEqual(action["data"]["date_to"], "2026-10-07")

    def test_the_roll_respects_record_rules(self):
        self.hearing(date=self.d("2026-10-04"))
        other_client = self.make_matter(client=self.client_b, name="Tigris v. Nahrain")
        self.hearing(other_client, date=self.d("2026-10-04"), attending_user_id=self.lawyer2.id)
