...
class TestMoneyConflict(MoneyCase):
    ...
    def setUpClass(cls):
        ...
        cls.rafidain = cls.env["legal.company"].create({
            "name": "شركة نينوى للمقاولات الإنشائية", "lawyer_id": cls.lawyer2.id, "lawyer_ids": [(6, 0, [cls.lawyer2.id])]})
        cls.opponent = Partner.create({"name": "مصرف الوركاء للاستثمار التجاري", "is_company": True})
    ...
    def test_opponent_who_is_our_client_is_a_conflict(self):
        result = self.check(["شركه نينوى للمقاولات الانشائيه"])
    ...
    def test_new_client_who_was_an_opponent_is_a_conflict(self):
        bank = self.env["legal.company"].create({"name": "مصرف الوركاء للاستثمار التجاري", "partner_id": self.opponent.id,
                                                  "lawyer_id": self.lawyer.id})
        result = self.check(["شخص غير معروف لدينا"], client=bank)
        self.assertEqual(result["decision"], "pending")
    ...
    def test_subsidiary_found_through_the_group(self):
        ...
        self.env["res.partner"].create({"name": "فرع تلعفر للإنشاءات الهندسية", "parent_id": group.id,
                                        "type": "other"})
        self.assertFalse(names_match("فرع تلعفر للإنشاءات الهندسية", self.rafidain.name))
        result = self.check(["فرع تلعفر للإنشاءات الهندسية"])
        self.assertEqual(result["decision"], "pending",
                         "the check must expand through commercial_partner_id / the parent company")
        subsidiary = self.env["res.partner"].create({"name": "شركة سنجار للتجارة والتوكيلات", "is_company": True,
                                                     "parent_id": group.id})
    ...
    def test_hits_are_redacted_for_those_who_cannot_open_them(self):
        bank = self.env["legal.company"].create({"name": "مصرف الوركاء للاستثمار التجاري", "partner_id": self.opponent.id,
                                                  "lawyer_id": self.lawyer.id})
        seen_by_lawyer = self.check(["شخص غير معروف لدينا"], client=bank)["hits"]
    ...
    def test_rerun_when_the_other_side_changes(self):
        ...
        self.assertEqual(matter.ldm_conflict_state, "pending")
        matter.counterparty_id = self.env["res.partner"].create({"name": "طرف ثالث في العقد"})
    ...
    def test_quick_create_with_a_conflict_stays_new(self):
        ...
        task_id = self.env["legal.task"].with_user(self.lawyer).create_from_template({
            ...
            "opponent_name": "شركة نينوى للمقاولات الإنشائية", "key_date": "2026-11-01"})
    ...
    def test_quick_create_without_a_conflict_starts(self):
        task_id = self.env["legal.task"].with_user(self.lawyer).create_from_template({
            "template_id": self.t_lawsuit.id, "legal_company_id": self.client_a.id,
            "opponent_name": "خصم جديد لم يرد سابقا", "key_date": "2026-11-01"})
        matter = self.env["legal.task"].browse(task_id)
